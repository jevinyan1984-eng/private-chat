...
const IMAGE_CONFIG = {
	...
	maxFileSize: 3 * 1024 * 1024, // 最大文件大小 3MB
...
export async function processImage(file, callback) {
	...
	if (file.size > IMAGE_CONFIG.maxFileSize) {
		alert(`图片过大！最大支持 ${IMAGE_CONFIG.maxFileSize / 1024 / 1024}MB`);
		return;
	}
	...
	img.onload = function() {
		...
		let dataUrl;
		// Export as webp with configured quality
		// 导出为 webp，使用配置的质量
		dataUrl = canvas.toDataURL(IMAGE_CONFIG.format, IMAGE_CONFIG.quality);
		callback(dataUrl);
	};
...
export function setupImagePaste(inputSelector) {
	...
	return {
		...
		addImage: addImage // 暴露添加图片方法
	};
}

// Setup image only send functionality (simplified)
// 设置仅图片发送功能（简化版）
export function setupImageOnlySend({
	inputSelector,
	attachBtnSelector,
	fileInputSelector,
	onSend
	...
	if (fileInput) {
		...
		fileInput.removeAttribute('multiple'); // 单张图片
	}
	
	if (attachBtn && fileInput) {
		// 点击附件按钮直接触发图片选择
		on(attachBtn, 'click', (e) => {
			e.preventDefault();
			e.stopPropagation();
			fileInput.click();
		});
	...
	// 注意：粘贴功能已由 setupImagePaste 处理，这里不需要重复
	// setupImagePaste 已经在 main.js 中调用，提供预览功能
}
