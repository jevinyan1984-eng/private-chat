...
// 导入访问密码配置
// Import access password configuration
import { ENABLE_ACCESS_PROTECTION } from './access-config.js';

// 从 util.image.js 中导入图片发送功能
// Import image send functions from util.image.js
import {
	setupImageOnlySend,
	processImage
} from './util.image.js';
...
// 访问权限验证成功后的初始化
// Initialize after access verification
function initializeApp() {
	...
	setupImageOnlySend({
		...
		onSend: (imageMessage) => {
			// 将选择的图片添加到预览中，和粘贴的图片一起发送
			if (imagePasteHandler && imageMessage && imageMessage.data && imagePasteHandler.addImage) {
				imagePasteHandler.addImage(imageMessage.data);
			}
		}
	});
...
// 当 DOM 内容加载完成后执行访问验证逻辑
// Run access verification logic when the DOM content is fully loaded
window.addEventListener('DOMContentLoaded', () => {
	...
	if (!ENABLE_ACCESS_PROTECTION) {
		// 如果未启用，直接初始化应用
		// If not enabled, initialize app directly
		initializeApp();
		return;
	}
	
	// 获取密码保护界面元素
	// Get password protection UI elements
	const passwordProtection = $id('password-protection');
	const passwordForm = $id('password-form');
	const passwordInput = $id('access-password');
	const passwordError = $id('password-error');
	
	// 如果没有找到密码保护界面元素，直接初始化应用
	// If password protection UI elements not found, initialize app directly
	if (!passwordProtection || !passwordForm || !passwordInput) {
		initializeApp();
		return;
	}
	
	// 密码表单提交处理（调用 API 验证）
	// Password form submission handler (call API to verify)
	passwordForm.addEventListener('submit', async (e) => {
		...
		try {
			...
			if (result.success) {
				// 密码正确，隐藏密码保护界面，显示登录界面
				// Correct password, hide password protection, show login screen
				...
				// Save access record to localStorage
				localStorage.setItem('nodecrypt_access_granted', 'true');
			} else {
				// 密码错误，显示错误信息
	...
	// 检查是否已经有访问记录
	// Check if there's already an access record
	const hasAccess = localStorage.getItem('nodecrypt_access_granted');
	if (hasAccess === 'true') {
		// 如果有访问记录，直接进入
		// If there's an access record, enter directly
		passwordProtection.style.display = 'none';
		const loginContainer = $id('login-container');
		if (loginContainer) {
			loginContainer.style.display = 'flex';
